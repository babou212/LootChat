...
const isCurrentUser = (odod: string) => {
  return user.value?.userId.toString() === odod
}
...
const handleViewScreenShare = (odod: string) => {
  const share = activeScreenShares.value.find(s => s.odod === odod)
  if (share) {
...
<template>
  <div v-if="voiceChannels.length > 0">
    <h3 v-show="!isCollapsed" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase px-2 mb-2">
      Voice Channels
    </h3>
    <div v-for="channel in voiceChannels" :key="channel.id" class="mb-1">
      ...
      <div v-if="!isCollapsed && currentChannelId === channel.id && participants.length > 0" class="ml-3 mt-1 space-y-0.5">
        <div
          ...
          :class="{
            ...
            'hover:bg-gray-100 dark:hover:bg-gray-700/50': !isUserSharing(participant.odod),
            'hover:bg-purple-100 dark:hover:bg-purple-900/30 cursor-pointer': isUserSharing(participant.odod)
          }"
          @click="isUserSharing(participant.odod) ? handleViewScreenShare(participant.odod) : undefined"
        >
          <div class="relative shrink-0">
            <div
              ...
              :class="{
                ...
                'ring-2 ring-purple-500 ring-offset-2 ring-offset-white dark:ring-offset-gray-800': isUserSharing(participant.odod) && !participant.isSpeaking,
                'ring-2 ring-gray-300 dark:ring-gray-600 ring-offset-2 ring-offset-white dark:ring-offset-gray-800': !participant.isSpeaking && !isUserSharing(participant.odod)
              }"
            >
              <UAvatar
                v-if="participant.avatar && getLoadedAvatarUrl(participant.odod)"
                :src="getLoadedAvatarUrl(participant.odod)"
                :alt="participant.username"
                size="xs"
              />
              <div v-else class="w-full h-full flex items-center justify-center text-xs font-semibold" :class="{ 'bg-green-600 text-white': participant.isSpeaking && !participant.isMuted, 'bg-gray-400 dark:bg-gray-600 text-white': !participant.isSpeaking || participant.isMuted }">
                {{ getParticipantInitials(participant.username) }}
              </div>
            </div>
            <div v-if="participant.isSpeaking && !participant.isMuted" class="absolute inset-0 rounded-full animate-ping bg-green-400 opacity-75" />
            <div v-if="participant.isMuted" class="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-red-500 border-2 border-white dark:border-gray-800 flex items-center justify-center">
              <UIcon name="i-lucide-mic-off" class="text-white text-[10px]" />
            </div>
            <div v-else-if="participant.isScreenSharing" class="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-purple-500 border-2 border-white dark:border-gray-800 flex items-center justify-center">
              <UIcon name="i-lucide-monitor" class="text-white text-[10px]" />
            </div>
          </div>
          <div class="flex-1 min-w-0 flex items-center gap-2">
            <span class="text-sm font-medium truncate" :class="{ 'text-green-700 dark:text-green-400': participant.isSpeaking && !participant.isMuted, 'text-gray-700 dark:text-gray-300': !participant.isSpeaking && !participant.isMuted, 'text-gray-500 dark:text-gray-500': participant.isMuted }">
              {{ participant.username }}
              <span v-if="isCurrentUser(participant.odod)" class="text-xs text-gray-500 dark:text-gray-400">(you)</span>
            </span>
            <div v-if="participant.isSpeaking && !participant.isMuted" class="flex items-center gap-0.5 h-3">
          ...
          <div class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
            ...
            />
            <UIcon :name="participant.isMuted ? 'i-lucide-mic-off' : participant.isSpeaking ? 'i-lucide-volume-2' : 'i-lucide-mic'" class="text-xs" :class="{ 'text-red-500': participant.isMuted, 'text-green-500': participant.isSpeaking && !participant.isMuted, 'text-gray-400': !participant.isSpeaking && !participant.isMuted }" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="!isCollapsed && isInVoice && connectedVoiceChannel" class="relative border-t border-gray-200 dark:border-gray-700 p-3 bg-gray-50 dark:bg-gray-900 mt-2">
      ...
      <div class="flex items-center gap-2 mb-2 px-2 py-1 bg-green-100 dark:bg-green-900/30 rounded">
        ...
        <span class="text-sm font-medium text-green-700 dark:text-green-300 truncate">{{ connectedVoiceChannel.name }}</span>
      </div>
      ...
      <div v-if="hasActiveScreenShare && activeScreenShares[0]?.odod" class="mt-2 p-2 bg-purple-100 dark:bg-purple-900/30 rounded flex items-center gap-2 cursor-pointer hover:bg-purple-200 dark:hover:bg-purple-900/50 transition-colors" @click="handleViewScreenShare(activeScreenShares[0].odod)">
        <UIcon name="i-lucide-monitor" class="text-purple-600 dark:text-purple-400" />
        <span class="text-xs text-purple-700 dark:text-purple-300 truncate flex-1">{{ activeScreenShares[0]?.sharerUsername || 'Someone' }} is sharing</span>
        <UButton
          color="primary"
          variant="soft"
          size="xs"
      ...
      <div v-if="showAudioSettings" class="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/50" @click.self="showAudioSettings = false">
        <div class="w-full max-w-lg p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-xl">
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-sm font-semibold text-gray-900 dark:text-white">
              ⚙️ Settings
            </h4>
            <button class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300" @click="showAudioSettings = false">
              <UIcon name="i-lucide-x" class="text-lg" />
            </button>
          </div>
          <div class="flex gap-1 mb-3 p-1 bg-gray-100 dark:bg-gray-700 rounded-lg">
            <button class="flex-1 px-3 py-1.5 text-xs font-medium rounded-md transition-all" :class="settingsTab === 'audio' ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-white shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'" @click="settingsTab = 'audio'">
              🎙️ Audio
            </button>
            <button class="flex-1 px-3 py-1.5 text-xs font-medium rounded-md transition-all" :class="settingsTab === 'screenshare' ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-white shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'" @click="settingsTab = 'screenshare'">
              🖥️ Screen Share
            </button>
          </div>
    ...
    <div v-if="isCollapsed && isInVoice" class="border-t border-gray-200 dark:border-gray-700 p-2 bg-green-50 dark:bg-green-900/20 mt-2">
      <UButton
        color="success"
        variant="soft"
        size="sm"
