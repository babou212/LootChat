...
# Volumes for persistent storage (optional - for stateful workloads)
...
resource "hcloud_volume" "backend_uploads" {
  name     = "${var.cluster_name}-backend-uploads"
  size     = var.uploads_volume_size
  location = var.location
  format   = "ext4"
